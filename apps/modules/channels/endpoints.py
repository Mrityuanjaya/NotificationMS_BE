...
from apps.modules.channels import schemas as channel_schemas, models as channel_models, constants as channel_constants
...
async def create_channel(channel_data: channel_models.ChannelInput):
    ...
    application = await ApplicationServices.get_active_application_by_id(channel_data.application_id)
...
async def get_channel(
    ...
    if current_user.role == common_constants.SYSTEM_ADMIN_ROLE:
        ...
        application_ids = await UserServices.get_active_application_ids_by_admin_id(current_user.id)
        response = await ChannelServices.get_limited_channels(
            page_no=page_no,
    ...
    for channel_instance in channel_instances:
        if channel_instance.deleted_at is not None:
            channels.append(
                {
                    "alias": channel_instance.alias,
                    "application_name": channel_instance.application.name,
                    "description": channel_instance.description,
                    "type": channel_instance.type,
                    "created_at": channel_instance.created_at.strftime("%d %B %Y, %I:%M:%S %p"),
                    "modified_at": channel_instance.modified_at.strftime("%d %B %Y, %I:%M:%S %p"),
                    "deleted_at": channel_instance.deleted_at.strftime("%d %B %Y, %I:%M:%S %p")
                }
            )
        else:
            channels.append(
                {
                    ...
                    "created_at": channel_instance.created_at.strftime("%d %B %Y, %I:%M:%S %p"),
                    "modified_at": channel_instance.modified_at.strftime("%d %B %Y, %I:%M:%S %p"),
                    "deleted_at": channel_instance.deleted_at
                }
            )
...
async def update_channel(channel_alias: str, channel_data: channel_models.ChannelUpdateInput):
    ...
    try: 
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"{error_field}:{error_msg}")
